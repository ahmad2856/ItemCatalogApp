#!/usr/bin/env python3
# This module Contain  the routes for the functioning of the App
...
# Loads Google Sign in API Client ID
CLIENT_ID = json.loads(
    open('client_secrets.json', 'r').read())['web']['client_id']

# Connecting to DB and create session
...
def gconnect():
    ...
    # If there an error in the accessing token info cancel
    if result.get('error') is not None:
        response = make_response(json.dumps(result.get('error')), 500)
        response.headers['Content-Type'] = 'application/json'
        return response

    # Check the access token is used for intended user
    ...
    # Check the access token is valid for this app.
    ...
    # Save access token in session for later use
    ...
    # Check if the user is there if not add new one
    user_id = get_user_id(data["email"])
    if not user_id:
        user_id = create_user(login_session)
    login_session['user_id'] = user_id

    # view  welcome screen when successfuly login
    output = ''
    output += '<h2>Welcome, '
...
# Log-out currently connected user
@app.route('/logout')
def logout():
...
def add_item():
    ...
    if 'username' not in login_session:
        ...
        # Check the item if already exist and display an error if already there
...
# Creating a new item by Category id
...
def view_item(item_id):
    ...
    if exists_item(item_id):
        ...
        flash('canot process this request right now')
        return redirect(url_for('home'))
...
def edit_item(item_id):
    ...
    if login_session['user_id'] != item.user_id:
        flash("Not authorised to access that page")
        return redirect(url_for('home'))
...
def edit_category(category_id):
    ...
    # If logged with user not authorized to edit redirect to homepage
...
#   If logged with user not authorized to edit category redirect to homepage
    if login_session['user_id'] != category.user_id:
        flash("canot process this request right now")
        return redirect(url_for('home'))
...
# Json Endpoints
# Return JSON of all items inside the catalog
...
def catalog_item_json(category_id, item_id):
    ...
    if exists_category(category_id) and exists_item(item_id):
        ...
        if item is not None:
            ...
            return jsonify(
                error='item {} does not belong to the category {}'
...
# Return JSON of a particular particular category in the catalog.
@app.route(
    '/api/v1/categories/<int:category_id>/item/JSON')
def category_items_json(category_id):
    """Return JSON of a particular category in the catalog."""
...
if __name__ == "__main__":
    app.secret_key = b' bKWsWgI9U_viJc05F8-ja0pE'
